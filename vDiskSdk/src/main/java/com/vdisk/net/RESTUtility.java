...
public class RESTUtility {
	...
	static public RequestAndResponse streamRequestAndResponse(
		...
		if (method == RequestMethod.GET) {
			...
			if (range > 0 && md5 != null) {
				req.setHeader("Range", "bytes=" + range + "-");// 璁剧疆涓嬭浇鑼冨洿 // Set download range
		...
		while (retry) {
			try {
				...
				if (response == null) {
					...
					// 濡傛灉鍙戠幇鏄柇鐐逛笅杞借繑鍥炵殑206鎴栬�302璺宠浆锛岀洿鎺ヨ繑鍥瀝esponse
	...
	static public HttpUriRequest streamHttpUriRequest(String url,
		...
		post.setHeader("Content-Type", "application/x-www-form-urlencoded");// 蹇呴』杩涜encode澶勭悊锛屽惁鍒欐姤閿�// Encode setting must be done to avoid an Error!
		byte[] data = null;
